...
State that invoking this slash command still uses the current Claude session even though the provider checks themselves do not generate routed model output. End successful reports with the exact next-step form `/ai-router:start-workflow <rough software goal>`.
